...
constexpr int ADC_PIN_TEMP = 0;
constexpr int AVG_SAMPLES = 20;

// Table voor temperaturen
...
// Convert raw values naar temperatuur
float rawToTemp(float raw) {
  if (raw <= tempTable[0][0]) {
    return tempTable[0][1];
  }
  if (raw >= tempTable[TABLE_SIZE - 1][0]) {
    return tempTable[TABLE_SIZE - 1][1];
  }
...
// Leest de raw data uit de sensor
...
// Bouw het bericht op
void buildPacket(CommunicationMessage *msg, uint8_t fc, uint8_t data, uint8_t data2 = 0x00) {
  msg->SOC = SOC_BYTE;
  msg->PL = sizeof(data + data2);  // one byte longer now
...
// Behandelt het ontvangen bericht
void handleReceivedPacket(CommunicationMessage *msg) {
  ...
  // Switch case voor de functie codes
  switch (msg->FC) {
    case FC_ACK:
...
void sendSensorData(uint8_t tempByte, uint8_t bpmByte) {
  ...
  for (uint8_t attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    while (millis() - t < ACK_TIMEOUT_MS) {
      if (ackReceived) {
        Serial.println("[SEND] ACK → succes!");
        return;
      }
      if (retransmitRequested) {
        Serial.println("[SEND] Retransmit...");
        break;
      }
      delay(10);
    }
